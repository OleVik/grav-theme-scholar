...
const cssColorRegExp = new RegExp(
  "^(#(?:[\\da-f]{3}){1,2}|rgb\\((?:\\d{1,3},\\s*){2}\\d{1,3}\\)|rgba\\((?:\\d{1,3},\\s*){3}\\d*\\.?\\d+\\)|hsl\\(\\d{1,3}(?:,\\s*\\d{1,3}%){2}\\)|hsla\\(\\d{1,3}(?:,\\s*\\d{1,3}%){2},\\s*\\d*\\.?\\d+\\))$"
);
const cssLengthRegExp = new RegExp(
  "^([-+]?0|[-+]?[0-9]*.?[0-9]+(%|cm|in|mm|pc|pt|px|rem))$"
);
...
module.exports = postcss.plugin("postcss-wcag-contrast", opts => {
  // set options
  const compliance =
    opts && "compliance" in opts ? String(opts.compliance).toUpperCase() : "AA";
  ...
  return (css, result) => {
    ...
    css.walkRules(rule => {
      ...
      const isLargeScale =
        fontsize >= 24 || (fontsize >= 14 * (96 / 72) && fontweight >= 700);
        ...
        if (!hasAAAContrast) {
          rule.warn(
            ...
            contrastRatio.toFixed(2) +
              ":1 is insufficient contrast between " +
              foreground +
              " and " +
              background +
              " - (WCAG 2.0 AAA requires " +
              minAAAContrastRatio +
              ":1)"
          );
        }
...
function getFontSize(cssLength) {
  ...
  const fontSizeScale =
    fontSizeUnit in cssLengthUnits ? cssLengthUnits[fontSizeUnit] : 1;

  return fontSize ? fontSize * fontSizeScale : 16;
}

function getFontWeight(cssWeight) {
  return cssWeight === "bold"
    ? 700
    : cssWeight === "normal"
    ? 400
    : parseInt(cssWeight) || 400;
}
